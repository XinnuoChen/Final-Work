...
synt.data <- synt.data.generation(real.data = real.data.1, cp.values = cp.values, minbucket.values = minbucket.values, m=2)$Syn

k.decision <- function(synt.data, k.real){
  m <- length(synt.data)
  k.synt <- numeric(m) #vector on guardo el nombre de k optims
  ssynt.data <- list(m) #dataframe escalat
  clust.synt <- list(m)
  ...
  for (i in 1:m){
    ...
    clust.synt[[i]] <- kmeans(ssynt.data[[i]], k.synt[i], nstart = 25) #resultado de clustering
  }
  clustering.result <- list("K optim" = k.synt, "Clustering" = clust.synt)
  i <- which(k$`K optim` == kreal) # kreal:k opt de les dades reals
...
# Em quedo nomes aquelles que tenen el mateix nombre de cluster
i <- which(k$`K optim` == 2) #substituir 2 per el k opt de les reals
clustering.result <- lapply(clustering.result, `[`, i)
```
